# -*- coding: utf-8 -*-
"""
Created on Sat Feb  1 09:26:07 2020
...
def train(rank, params, shared_model, optimizer):
    ...
    while True:
        ...
        if done: 
            cx = Variable(torch.zeros(1,265))
            hx = Variable(torch.zeros(1,265))
        else:
            cx = Variable(cx.data)
            hx = Variable(hx.data)
        values = []
        ...
        for step in range(params.num_steps):
            ...
            done = (done or episode_length >= params.max_episode_lenght)
        ...
        value_loss = 0
        R = Variable(R)
        #gae = A(a,s) = Q(a,s) - V(s) going to initialise this
        gae = torch.zeros(1,1)
        for i in reversed(range(len(rewards))):
            R = params.gamma * R + rewards[i] # R = r_0 +gamma * r_1 + gamma^2 * r_2 ... 
